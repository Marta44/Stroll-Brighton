...
- Another challenge has been building the carousel with the "card places" generated by google map. My mistake was that I was calling placesSlider.owlCarousel() immediately on page load, before the user was doing any search. On page load is empty so it couldn't display anything. I moved placesSlider.owlCarousel() right after cardContainer.innerHTML with the card. In this way when the user clicked on the first category the carousel was displayed and worked fine. The other problem was that when another category was selected, and the results were cleared, the carousel stopped working.
